...
func newHttpServer(name string, addr string, cfg configx.Config, logger logx.Logger) *gap.Server {
	...
	srv.AddRouter("/trace", trace(cfg, logger))
	return srv
}

func trace(cfg configx.Config, logger logx.Logger) gap.HandlerFunc {
	return func(c *gap.Context) {
		addr := getCfgValue(cfg, "service.test.addr", logger)
		if addr == "" {
